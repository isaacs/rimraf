...
const rimrafWindows = async (path, opts) => {
  ...
  if (!Array.isArray(entries)) {
    ...
    return await tmpUnlink(path, opts.tmp, unlinkFixEPERM)
  }

  await Promise.all(entries.map(entry =>
    rimrafWindows(resolve(path, entry), opts)))

  return await tmpUnlink(path, opts.tmp, rmdir)
}
...
const rimrafWindowsSync = (path, opts) => {
  ...
  if (!Array.isArray(entries)) {
    ...
    return tmpUnlinkSync(path, opts.tmp, unlinkFixEPERMSync)
  }

  for (const entry of entries)
    rimrafWindowsSync(resolve(path, entry), opts)

  return tmpUnlinkSync(path, opts.tmp, rmdirSync)
}
